      ...
      signUp: async (email, password) => {
        try {
          ...
          try {
            ...
            console.log('[AUTH] calling backend /api/signup payload:', { email: em, firebaseUid: uid });
            ...
            if (isUnverifiedExists) {
              console.log('[AUTH] Backend indicates existing unverified user — attempting backend resend (/api/signup/resend)');
              try {
                ...
                if (resendResp && resendResp.status >= 200 && resendResp.status < 300) {
                  ...
                  console.warn('[AUTH] Backend resend returned non-OK', resendResp.status, resendResp.data);
                  set({
                    error:
